...
static void init_model(const char *initpath, HMM *hmm) {
   FILE *fp = open_or_die(initpath, "r");
   loadHMM(hmm, initpath);
   fclose(fp);
}

static void print_model(const HMM *hmm) {
   int i, j;
   printf("\n");
   printf("initial: %d\n", hmm->state_num );
   for( i = 0 ; i < hmm->state_num - 1; i++ )
   ...
   for( i = 0 ; i < hmm->observ_num ; i++ ){
      for( j = 0 ; j < hmm->state_num - 1 ; j++ )
         printf("%.5lf ", hmm->observation[i][j] );
      printf("%.5lf\n", hmm->observation[i][hmm->state_num - 1]);
   }
}
